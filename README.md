...
사용자의 요청에 맞는 주소로 찾아가서 인터넷 컨텐츠를 검색·열람하여
...
- **http** (Hypertext Transfer Protocol)
  클라이언트와 서버 간 웹 페이지 등의 자원을 통신하는 규약.
...
네트워크 상에서 인터넷을 접속할 때 다른 컴퓨터와 구별될 수 있도록 하는 고유 번호.
...
마크업 언어란 태그 방법 체계를 의미하며, 태그 등을 이용해서 문서나 데이터의
구조를 기술하는 언어이다.

### 2. CSS (Cascading Style Sheets)

구체적으로 어떤 스타일로 요소가 표시되는지를 정하는 규칙이다.
HTML 문서를 구조화(레이아웃)하는 것 뿐 아니라 꾸미기도 할 수 있다.
동일한 디자인을 사용한 문서가 여러 개 있다면, 변경 시 모두 하나씩 수정해야 한다.
CSS는 이러한 문제를 해결함과 동시에 웹 페이지에서의 내용과 스타일을 분리하고,
역할도 분리해준다.
...
- **여는 태그** : 태그의 이름(예를 들어, `p`)을 적고 열고 닫는 꺽쇠 괄호로 구성된다.
- **내용** : 해당 태그에 적용될 내용이며, 단순한 텍스트를 의미한다.
- **닫는 태그** : 태그의 이름 앞에 슬래시(`/`)가 있다.
...
특히 `id`와 `class` 속성은 스타일에 관련된 내용이나 기타 연산 등의 내용을 위해
...
3. 속성 값은 따옴표 안에 작성한다 (따옴표는 생략할 수 있다).
...
- 코드 상에 한 줄로 이어 써도 화면 상에서는 뒤에 있는 요소가 들어갈 공간이 없기 때문에 아래줄로 내려간다.
...
- 영역이 정확히 구분되어 있기 때문에, `width`와 `height` 속성을 수정할 수 있다.
- `margin-top`, `margin-bottom`, `padding-top`, `padding-bottom` 속성도 잘 적용된다.
...
- 영역이 정확히 구분되어 있지 않기 때문에, `width`와 `height`를 임의로 부여할 수 없다.
- `margin-top`, `margin-bottom`은 적용되지 않는다.
- `padding-top`, `padding-bottom`은 적용은 되지만 시각적으로는 적용되지 않은 것처럼 보여서 레이아웃 제어에는 적합하지 않다.

### 인라인-블록 요소

- `button`, `input`, `select` …
- 인라인 요소와 동일한 영역(내용만큼)을 가지지만 본인만의 영역을 정확히 가지고 있다.
- `width`, `height`, `margin`, `padding` 모두 잘 적용된다.
